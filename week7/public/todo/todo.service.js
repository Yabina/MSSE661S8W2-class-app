...
class ToDo {
  ...
  _renderListRowItem = (task) => {
    ...
    editBtn.addEventListener('click', this._editEventHandler(task));
  ...
  //   const newTask = { ...task, task_id, creeated_date };
  ...
  _addTaskEventHandler = () => {
    ...
    const {
      newTask,
      newTaskEl
    } = this._createNewTaskEl(task); // add task to list
  ...
  /**
   * DOM Event handler helper for updating a task in the DOM.
   *
   * @param {Object} task - The task object to be updated
   */

  _editEventHandler = (task) => () => {
    const taskName = prompt('Enter the updated task name:', task.task_name);
    if (taskName === null || taskName === '') return; // If user cancels or enters empty string

    const status = prompt('Enter the updated status:', task.status);
    if (status === null) return; // If user cancels
    ...
    /**
   * Pure function for updating a task.
   *
  ...
  updateTask = async (updatedTask) => {
      try {
        const user_id = storageHasData() ? getStorage('user_id') : '';
        await this.tasksService.updateTask(user_id, updatedTask);
  ...
  deleteTask = async (taskId) => {
    try {
      const user_id = storageHasData() ? getStorage('user_id') : '';
      const res = await this.tasksService.deleteTask(user_id, taskId);
  ...
  render = async () => {
    const user_id = storageHasData() ? getStorage('user_id') : '';
    const tasks = await this.tasksService.getAllTasks(user_id);
    try {
      if (tasks.length) {
        this.tasks = tasks;
        this._renderList();
